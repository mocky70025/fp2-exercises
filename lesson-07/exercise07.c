/* ねむいねむいねむいねむいねむいねむいねむいねむいねむいねむいねむいねむいねむいねむい
 * 基礎プログラミングII — 第7回 演習
...
int main(void) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        scanf("%d",  &patterns[i].pin/* ここを書く */);
        printf("パターン%d — 点灯時間(ms): ", i + 1);
        scanf("%d", &patterns[i].duration/* ここを書く */);
    }
    ...
    for (int i = 0; i < n; i++) {
        printf("パターン%d: ピン%dを%dms点灯\n",
               i + 1, patterns[i].pin, patterns[i].duration/* ここを書く */);
    }
